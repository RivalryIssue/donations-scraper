...
def get_data(lanternurl: str, dailyurl: str):
    ...
    lantern_money = lantern_soup.find_all(
        class_="single-project-top-dollar")[0].text.strip()
    ...
    response = requests.request(
        "POST", daily_url, headers=headers, data=payload)
    daily_money = round(float(response.json()[
        "data"]["storyById"]["storyInitiativesByStoryId"]["nodes"][0]["totalAmountOfGifts"]))

    return {
        "data": {
            "lantern": lantern_money,
            "daily": "$" + "{:,}".format(daily_money),
        }
    }
